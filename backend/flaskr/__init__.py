...
def create_app(test_config=None):
    ...
    def get_questions():
        ...
        try:
            all_questions = db.session.query(Question).order_by(Question.id).offset(start_page).limit(
    ...
    def bad_request(error):
        return jsonify(dict(success=False, error=400, message='bad request')),400

    @app.errorhandler(404)
    def resource_not_found(error):
        return jsonify(dict(success=False, error=404, message='resource not found')),404

    @app.errorhandler(422)
    def unprocessable(error):
        return jsonify(
            dict(success=False, error=422, message='The server understands the content type of the request entity')
        ),422
